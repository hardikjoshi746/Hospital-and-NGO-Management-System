...
public class MayorWorkAreaJPanel extends javax.swing.JPanel {
    ...
    public void populateWorkRequestTable()
    {
        ...
        try
        {
        communitydetailsmap = getHelpNeededData();
        }
    ...
    public HelpNeededOrganization getHelpNeededOrg()
    {
        HelpNeededOrganization helpNeededOrg = null;
        ...
        for(Network network : ecoSystem.getNetworkList())
        {
         if(network.equals(userAccount.getNetwork()))
         {
          for(Enterprise enterprise : network.getEnterpriseDirectory().getEnterpriseList())
          {
            ...
            {
             if(organization instanceof HelpNeededOrganization)
             {
              helpNeededOrg = (HelpNeededOrganization)organization; 
             }
            }
            }
          }
        ...
        return helpNeededOrg;
    }
    
    public HashMap<String,Integer> getHelpNeededData()
    {
        ...
        HelpNeededOrganization helpNeededOrganization = getHelpNeededOrg();
        if(helpNeededOrganization == null)
        {
        JOptionPane.showMessageDialog(null, " Help Seeker Organization does not exist!","warning", JOptionPane.WARNING_MESSAGE);
        return null;     
        }
        if(helpNeededOrganization.getPersonDirectory().getCustomerLsit().isEmpty())
        {
        JOptionPane.showMessageDialog(null, "No Help Seekers Registered!","warning", JOptionPane.WARNING_MESSAGE);
        return null; 
        }
        int totalSeniors = helpNeededOrganization.getPersonDirectory().getCustomerLsit().size();
      HashMap<String,Integer> seniorHeartPatientMap = new HashMap<>();
       for(HelpNeeded helpNeeded : helpNeededOrganization.getPersonDirectory().getCustomerLsit())
          {
           if(helpNeeded.isHasCardiacProb())     
           {
            seniorCardiacProb ++;  
           } 
           if(!helpNeeded.isHasCardiacProb())
           ...
           if(helpNeeded.isHasSensorDevice())
           {
           ...
           if(helpNeeded.isSendDataToDoctor())
           {
            seniorsSendDataToDoctor ++;   
           } 
           if(helpNeeded.isShareVitalInfo())
           {
            seniorsShareVitalInfo ++;   
           } 
           avgAge+=helpNeeded.getAge();
          } 
